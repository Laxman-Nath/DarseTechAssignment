...
public class OpenApiContextListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent sce) {
		...
		openAPI.path("/addBook",
				new PathItem()
						.post(new Operation().summary("Add book to database")
								...
								.responses(new ApiResponses().addApiResponse("200", new ApiResponse().description("Book added"))
										.addApiResponse("500", new ApiResponse().description("Error adding book")))
								.requestBody(
		...
		openAPI.path("/deleteBook",
				new PathItem().get(new Operation().summary("Delete book from database")
						.description("This endpoint will delete a book from the database by its unique ID.")
						.parameters(Arrays.asList(new QueryParameter().name("id")
								.description("The unique identifier of the book to be deleted").required(true)
								.schema(new IntegerSchema())))
						.responses(
								new ApiResponses().addApiResponse("200", new ApiResponse().description("Book deleted"))
		...
		openAPI.path("/editBook",
				new PathItem()
						.post(new Operation().summary("edit book in database")
								...
								.parameters(Arrays.asList(new QueryParameter().name("id")
										.description("The unique identifier of the book to be edited").required(true)
										.schema(new IntegerSchema())))
								.responses(new ApiResponses()
										.addApiResponse("200", new ApiResponse().description("Book edited"))
										.addApiResponse("500", new ApiResponse().description("Error editing book"))))
						.get(new Operation().summary("show form for editing book")
								.description("This endpoint display form for editing book .")
								.responses(new ApiResponses().addApiResponse("404",
										new ApiResponse().description("Invalid id of book")))));
